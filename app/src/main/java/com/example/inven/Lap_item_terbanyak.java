...
public class Lap_item_terbanyak extends AppCompatActivity {
...
//----------------------------------------------------------------------------
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.lap_item_terbanyak);

        barChart = findViewById(R.id.bar_chart);

        List<BarEntry> entries = getChartData();

        // Membuat dataset dan mengisi datanya dengan data yang didapat
        BarDataSet dataSet = new BarDataSet(entries, "Label");
        BarData barData = new BarData(dataSet);

        // Atur dataset ke chart dan tampilkan
        ...
        context = Lap_item_terbanyak.this;
    ...
    private List<BarEntry> getChartData() {
        ...
        // Misalnya, kita tambahkan beberapa data secara statis
        entries.add(new BarEntry(1, 50)); // Data ke-1
        entries.add(new BarEntry(2, 80)); // Data ke-2
        entries.add(new BarEntry(3, 60)); // Data ke-3
        entries.add(new BarEntry(4, 70)); // Data ke-4
        return entries;
    }
    ...
    private final Runnable m_Runnable = new Runnable() {
        public void run() {
            ...
            Lap_item_terbanyak.this.mHandler.postDelayed(m_Runnable, 2000);
        }
...
//        pDialog.setCanceledOnTouchOutside(false);
    ...
    private void KasAdapter3(){
        ...
        //Log.d("link", LINK );
        AndroidNetworking.post( Config.host + "lap_gabung_revisi_item.php" )
...
//                                map.put("pembayaran",       responses.optString("pembayaran"));
    ...
    private void Adapter2() {
        ...
        //List<Integer> colors = new ArrayList<>(); // Daftar warna untuk setiap batang

        for (int i = 0; i < aruskas.size(); i++) {
            HashMap<String, String> data = aruskas.get(i);
            String qtyString = data.get("qty");

            // Cek apakah qtyString adalah angka yang valid
            if (isNumeric(qtyString)) {
                // Konversi qtyString ke float untuk digunakan sebagai sumbu Y
                float yValue = Float.parseFloat(qtyString);

                // Tambahkan nilai ke dalam BarEntry
                entries.add(new BarEntry(i, yValue)); // Gunakan nomor urut sebagai nilai sumbu X

                // Tambahkan nomor urut sebagai label untuk sumbu X
                labels.add(String.valueOf(i + 1));

                // Tambahkan warna biru muda ke dalam daftar warna
                //colors.add(Color.BLUE);
            } else {
                // Jika bukan angka yang valid, lewati entri ini
                Log.e("Adapter2", "Skipping entry with non-numeric qty: " + qtyString);
            }
        }

        // Buat dataset untuk diagram batang
        BarDataSet dataSet = new BarDataSet(entries, "Label");

        // Atur warna untuk setiap batang dalam dataset
        //dataSet.setColors(colors); // Terapkan satu warna ke semua batang dalam dataset

        // Buat data untuk diagram
        BarData barData = new BarData(dataSet);

        // Atur data ke dalam diagram
        barChart.setData(barData);
    ...
    // Deklarasikan variabel indeks terkait dengan setiap item dalam daftar
    private int selectedItemIndex = -1; // Default: tidak ada item yang dipilih

    // Di dalam metode Adapter3(), tambahkan kode untuk menangani klik item dalam daftar
    private void Adapter3() {
        SimpleAdapter simpleAdapter = new SimpleAdapter(this, aruskas, R.layout.list_data_laporan_grafik,
        ...
        listinputperdana1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                // Tetapkan indeks item yang dipilih
    ...
    // Buat metode untuk memperbarui tampilan diagram
    private void updateChart() {
        // Atur batang yang dipilih
        barChart.highlightValue(selectedItemIndex, 0); // Highlight batang dengan indeks selectedItemIndex

        // Atur ulang variabel selectedItemIndex setelah pembaruan diagram
        selectedItemIndex = -1; // Reset selectedItemIndex agar tidak ada item yang terpilih
    }
